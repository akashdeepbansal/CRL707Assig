...
%[six1,fs1,nbits1]=wavread('pratik_four');
[six1,fs1] = audioread('4.wav');
N1=length(six1);
m1=0:N1-1;
t1=m1/fs1;
...
xlabel('time(in sec)');xlim([min(t1) max(t1)]);ylabel('Amplitude');title('Time waveform for the original utterance - four');
subplot(2,1,2);plot(t2,six2,'k');
xlabel('time(in sec)');xlim([min(t2) max(t2)]);ylabel('Amplitude');title('Time waveform for the downsampled utterance - four');

%\\Part-1.b :extracting 20 ms of voiced and unvoiced speech and plotting them 

% CHANGE HERE
n_unvoiced=701:860;% 20ms segment of 8 KHz speech means 160 samples
unvoiced_six=six2(n_unvoiced);
t_unvoiced=n_unvoiced/fs2;

n_voiced=1901:2060; % 20ms segment of 8 KHz speech means 160 samples
...
title('Voiced Segment for the utterance - four');%ylim([-0.03 0.03]);
subplot(2,1,2);
plot(t_unvoiced,unvoiced_six,'k');
xlabel('time(in sec)');xlim([min(t_unvoiced) max(t_unvoiced)]);ylabel('Amplitude');
title('Unvoiced Segment for the utterance - four');%ylim([-0.005 0.005]);
...
figure()
subplot(2,1,1);plot(f,yv,'k'); xlabel('Frequency(Hz)');ylabel('Magnitude(dB)');ylim([-60 15]);
title('Magnitude spectrum of Voiced Segment of the utterance - four');
subplot(2,1,2);plot(f,yuv,'k');xlabel('Frequency(Hz)');ylabel('Magnitude(dB)');ylim([-82 0]);
title('Magnitude spectrum of Unvoiced Segment of the utterance - four');
